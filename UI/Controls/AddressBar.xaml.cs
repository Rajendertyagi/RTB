...
namespace TB_Browser.UI.Controls
{
    public partial class AddressBar : UserControl
    {
        ...
        public AddressBar(IBrowserService svc)
        {
            ...
            _svc.CanGoBackChanged += (_, b) => BackBtn.IsEnabled = b;
            _svc.CanGoForwardChanged += (_, b) => FwdBtn.IsEnabled = b;
        }
        ...
        private void Bookmark_Click(object s, RoutedEventArgs e) { var btn = (Button)s; btn.Content = btn.Content.ToString() == "☆" ? "⭐" : "☆"; Logger.Info("AddressBar", $"Bookmark toggled: {btn.Content}"); }
