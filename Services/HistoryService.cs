...
public class HistoryService
{
    ...
    {
        _queue.Enqueue(new HistoryEntry 
        { 
            Url = url, 
            Title = title, 
    ...
    public async Task FlushAsync()
    {
        if (!_purgeDone) 
        { 
            await _repository.PurgeOlderThanAsync(DateTime.UtcNow.AddDays(-30)); 
            _purgeDone = true; 
        }
    ...
    private static async Task RetryAsync(Func<Task> action, int maxRetries)
    {
        for (int i = 1; i <= maxRetries; i++) 
            ...
            catch (Exception ex) { LoggingService.Error($"Retry {i}", ex); if (i == maxRetries) throw; await Task.Delay(100 * (int)Math.Pow(2, i)); }
    }
}
